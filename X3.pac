...
//   ✅ Region ID Activation (1001-1012)
...
// 🚀 v12 - Multi-Proxy Failover (لوبي + تجنيد)
var PROXY_POOL = [
...
// 🎮 بروكسي ومنفذ واحد للمباريات
var MAT = "SOCKS5 46.185.131.218:20001"
...
var JO_REGION_IDS = [
    "1001", "1002", "1003", "1012", "1005", "1006",
    "1007", "1008", "1009", "1010", "1011", "1004"
];

var BACKUP_REGION_IDS = ["2001", "2002", "2003"];
...
// ╔══════════════════════════════════════════════════════════╗
// ║  ③ نطاقات أردنية IPv4 EXPANDED v12                      ║
// ╚══════════════════════════════════════════════════════════╝
...
// ════════════════════════════════════════════════════════════
//  🚀 v12 - Region ID Activation
//  يفحص الـ URL كاملاً عن أرقام مناطق الأردن (1001-1012)
//  أو مناطق الـ backup (2001-2003) عند الحاجة
// ════════════════════════════════════════════════════════════
function hasJordanRegion(url) {
    var u = url.toLowerCase();
    // فحص أرقام المناطق الأردنية
    for (var i = 0; i < JO_REGION_IDS.length; i++) {
        var id = JO_REGION_IDS[i];
        // يبحث عن النمط في الـ URL: region=1001 أو regionid=1001 أو /1001 أو _1001
        if (u.indexOf("region=" + id) !== -1 ||
            u.indexOf("regionid=" + id) !== -1 ||
            u.indexOf("region_id=" + id) !== -1 ||
            u.indexOf("server=" + id) !== -1 ||
            u.indexOf("serverid=" + id) !== -1 ||
            u.indexOf("/" + id + "/") !== -1 ||
            u.indexOf("/" + id + "?") !== -1 ||
            u.indexOf("_" + id + "_") !== -1 ||
            u.indexOf("." + id + ".") !== -1) {
    ...
    // فحص مناطق الـ backup كـ smart fallback
    if (SMART_FALLBACK) {
        for (var j = 0; j < BACKUP_REGION_IDS.length; j++) {
            var bid = BACKUP_REGION_IDS[j];
            if (u.indexOf("region=" + bid) !== -1 ||
                u.indexOf("regionid=" + bid) !== -1 ||
                u.indexOf("region_id=" + bid) !== -1 ||
                u.indexOf("server=" + bid) !== -1) {
                return true;
            }
        }
    }
...
function FindProxyForURL(url, host) {
    ...
    // ════════════════════════════════════════════
    //  ③ Region ID Activation - أرقام المناطق
    //     أي طلب فيه رقم منطقة أردنية → LOB
    // ════════════════════════════════════════════
    if (hasJordanRegion(url)) {
        return LOB;
    }
